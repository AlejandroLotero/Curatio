...
export default function WebCheckoutPage() {
  ...
  const validatePaymentStep = () => {
      ...
      if (brand === "American Express") {
        if (digitsLength < 15) {
          nextErrors.cardNumber =
            "La tarjeta American Express debe tener 15 dígitos.";
        }
    ...
    if (paymentMethod === "transfer") {
      ...
      if (!paymentForm.transferDocumentNumber.trim()) {
        nextErrors.transferDocumentNumber =
  ...
  const handleProcessPayment = async () => {
    ...
    try {
      ...
      if (hasInvalidMedicationId) {
        setApiError(
          "No se pudo identificar correctamente uno o más medicamentos del carrito."
        );
        setPaymentStatus("rejected");
        setStatusMessage(
          "No se pudo identificar correctamente uno o más medicamentos del carrito."
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      console.error("Error descargando factura:", error);
      setApiError(error?.error?.message || "No se pudo descargar la factura.");
    }
  };
    ...
    return (
      <div className="min-h-screen px-4 py-8 text-label">
        <div className="max-w-3xl mx-auto bg-white/70 backdrop-blur-md rounded-3xl shadow-xl p-8">
          <div className="flex items-center gap-3 mb-4">
            <CircleAlert className="size-6" />
            <h1 className="text-2xl font-bold">Checkout restringido</h1>
          </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1.35fr_0.95fr] gap-6">
          ...
          <div className="space-y-6">
            <section className="rounded-3xl bg-white/70 backdrop-blur-md shadow-xl p-6">
              ...
              <div className="space-y-3">
                <SummaryRow label="Cliente" value={getUserName(user) || "—"} />
                <SummaryRow label="Correo" value={getUserEmail(user) || "—"} />
                <SummaryRow label="Ítems" value={String(cartCount)} />
                <SummaryRow
                  label="Subtotal"
                  value={`$${formatCurrency(cartSubtotal)}`}
          ...
          {paymentStatus === "approved" && checkoutSale ? (
            <div className="rounded-2xl border border-border-strong bg-white/60 p-4">
              ...
              <div className="space-y-2 text-sm">
                <SummaryRow
                  label="Factura"
                  value={checkoutSale.invoice_number}
                />
                <SummaryRow label="Fecha" value={checkoutSale.sale_datetime} />
                <SummaryRow
                  label="Cliente"
                  value={checkoutSale.customer?.name || ""}
                />
                <SummaryRow
                  label="Método de pago"
                  value={checkoutSale.payment_type}
                />
                <SummaryRow label="Estado" value={checkoutSale.status} />
                <SummaryRow
                  label="Total"
...
function SummaryRow({ label, value, strong = false }) {
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <span
        className={
          strong ? "font-bold text-label text-right" : "text-label text-right"
        }
      >
        {value}
      </span>
