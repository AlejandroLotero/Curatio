...
/**
 * Mapea los nombres de error del backend
 * a los nombres de campo que usa el formulario.
 */
function mapBackendErrorsToUiFields(backendFields = {}) {
  const fieldMap = {
    nombre: "nombre",
...
export default function ProductsPage() {
  ...
  /**
   * Archivo real seleccionado para imagen.
   * Ya no guardamos una URL fake.
   */
  ...
  /**
   * Carga presentaciones según forma farmacéutica.
   */
  const handleFormaChange = async (e) => {
    const formaId = e.target.value;
  ...
  /**
   * Toma los valores reales del formulario HTML
   * y los convierte en un objeto compatible con el adapter.
   */
  const buildFormValues = () => {
    const htmlFormData = new FormData(formRef.current);

    return {
      nombre: htmlFormData.get("nombre") ?? "",
      formaFarmaceutica: htmlFormData.get("formaFarmaceutica") ?? "",
      presentacion: htmlFormData.get("presentacion") ?? "",
      concentracion: htmlFormData.get("concentracion") ?? "",
      descripcion: htmlFormData.get("descripcion") ?? "",
      viaAdministracion: htmlFormData.get("viaAdministracion") ?? "",
      laboratorio: htmlFormData.get("laboratorio") ?? "",
      lote: htmlFormData.get("lote") ?? "",
      fechaFabricacion: htmlFormData.get("fechaFabricacion") ?? "",
      fechaVencimiento: htmlFormData.get("fechaVencimiento") ?? "",
      stock: htmlFormData.get("stock") ?? "",
      precioCompra: htmlFormData.get("precioCompra") ?? "",
      precioVenta: htmlFormData.get("precioVenta") ?? "",
      proveedor: htmlFormData.get("proveedor") ?? "",
      estado: htmlFormData.get("estado") ?? "",
  ...
  const handleButtonSubmit = async (e) => {
     ...
     * La imagen es opcional, así que no debe romper si viene null.
     */
    ...
    try {
      ...
      /**
       * Aquí ya construimos FormData real.
       */
      const apiPayload = adaptMedicationPayloadFromForm(values);
  ...
  /**
   * Dispara el submit real cuando el usuario confirma.
   */
  const handleConfirmSave = () => {
    formRef.current?.requestSubmit();
  };
  ...
  /**
   * Recibe el File real desde ProductFileInput.
   */
    ...
    <div className="flex items-center justify-center min-h-screen text-label px-4 py-6 sm:px-6 sm:py-8 w-full min-w-0 overflow-x-hidden">
      ...
      >
        ...
        <div className="col-span-full mt-3">
          ...
          />
          {errors.imagen ? (
            <p className="text-mostsmall text-red-600 mt-1">{errors.imagen}</p>
          ) : null}
        </div>
