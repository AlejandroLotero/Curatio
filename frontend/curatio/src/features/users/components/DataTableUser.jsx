...
// Utilidades principales de TanStack Table
import {
  useReactTable,
  getCoreRowModel,
  flexRender,
 ...
 * DataTableUser
 * -------------
 * Tabla reutilizable para usuarios.
 *
 * Ahora recibe filtros reales desde el padre:
 * - search
 * - role
 * - statusFilter
 * - documentFilter
 *
 * El backend es quien devuelve los datos ya filtrados.
 */
export default function DataTableUser({
  ...
  // Filtro global visual para TanStack
  // Lo mantenemos sincronizado con "search"
    ...
    <div className="flex items-center justify-center p-4">
      <div className="w-full max-w-6xl space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {/* Selector de filas visibles por página local */}
          <select
            value={table.getState().pagination.pageSize}
            onChange={(e) => table.setPageSize(Number(e.target.value))}
            className="border rounded px-2 py-2 text-black"
