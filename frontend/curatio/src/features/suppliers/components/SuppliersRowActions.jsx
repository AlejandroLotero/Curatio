...
// Componente que renderiza las acciones de cada fila de supplier.
// Recibe como prop el objeto supplier ya adaptado al shape del frontend.
export default function SuppliersRowActions({ supplier }) {
  // Hook que permite redirigir a otra ruta desde código.
  const navigate = useNavigate();

  /**
   * Acción para visualizar el detalle del proveedor.
   *
   * Se navega usando el NIT, que en backend corresponde a la PK del proveedor.
   */
  const handleView = () => {
    const nit = supplier?.nit;
   ...
   *
   * Se usa el NIT como identificador estable del recurso para evitar
   * inconsistencias entre id y nit.
   */
  const handleEdit = () => {
    const nit = supplier?.nit;
